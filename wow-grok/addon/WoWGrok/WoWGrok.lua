-- WoWGrok: talk to local Grok sessions from inside WoW, without reloading.
--
...
--        slot addons (WoWGrok_S001..S200); we load a fresh slot from a timer.
...
--
-- Chats: each chat is its own Grok conversation (like a separate terminal) with its own
...
local ROLE_STYLE = {
	...
	grok = { label = "Grok", color = { 1.00, 0.82, 0.25 }, bg = { 0.85, 0.70, 0.30, 0.10 } },
...
local keyCatcher = CreateFrame("Frame", "WoWGrokKeyCatcher", UIParent)
keyCatcher:Hide()
...
function WoWGrok.ArmAutoRefresh()
	...
	if not UseReloadTransport() then return end
	...
	-- Short interval when capture paused / reload transport so the second
	-- Refresh after send-reload is not a long wait (Inbox may land after the first reload).
	local wait = db.settings.interval or 20
	if run.capturePaused or db.settings.mode ~= "pixel" then
		wait = math.min(wait, 8)
	end
	WoWGrok.nextAutoRefresh = GetTime() + wait
	keyCatcher:Show()
end
...
local function EnsureStrip()
	...
	strip = CreateFrame("Frame", "WoWGrokStrip", UIParent)
...
local function CheckSignal(kind, id)
	...
	return SoundValid(string.format("Interface\\AddOns\\WoWGrok\\%s\\%03d.wav", kind, SlotNumber(id)))
end

-- Heartbeat: the bridge flips act/NNN/kk.wav for the k-th action of message NNN.
local function ActPath(id, k)
	return string.format("Interface\\AddOns\\WoWGrok\\act\\%03d\\%02d.wav", SlotNumber(id), k)
end
...
local function PresencePath(k)
	return string.format("Interface\\AddOns\\WoWGrok\\presence\\%04d.wav", k)
end
...
function WoWGrok.UpdateDot()
	local state, _, _, _, tip = WoWGrok.BridgeState()
...
function WoWGrok.IsConnected()
	...
	-- Reload transport never gates on the pixel light.
	if db.settings.mode ~= "pixel" or run.capturePaused then return true end
...
function WoWGrok.Connect()
	...
	if UseReloadTransport() then
		-- Reload/SV: mark connected via presence; Refresh/Enter reads Inbox.
		run.awaitingCaptureHint = nil
		run.connectingAt = nil
		print("|cff88ccffWoW Grok|r Connect: presence/reload chat (capture paused or reload mode)")
		WoWGrok.Render()
		return
	end
	run.lastHelloAt = nil
...
function WoWGrok.CheckConnection()
	if run.connectingAt then
		if WoWGrok.IsConnected() then
			...
			if c and c.id == queued.chat and not c.pendingId then
				...
				WoWGrok.Send(queued.text, queued.allow)
			end
	...
	elseif run.connectFailed and WoWGrok.IsConnected() then
		run.connectFailed = nil
	end
...
function WoWGrok.UpdateConnect()
	if not ui.connect or not ui.send then return end
	local connected = WoWGrok.IsConnected()
	...
	if run.connectingAt then
		...
		ui.connect:SetText(WoWGrok.BridgeState() == "stale" and "Reconnect" or "Connect")
		ui.connect:Enable()
	end
end
...
local function ActivityLine(chat)
	...
	if a and not a.unreliable then
		...
		if a.last then
			...
			if quiet > 120 then s = s .. " (quiet for a while - stuck? /wow-grok cancel)" end
...
local function Tick()
	...
	if moved then WoWGrok.Render() end
	for _, c in ipairs(db.chats) do
...
local function ContextToSend(room)
	local ctx = db.settings.context and WoWGrok.GameContext() or ""
...
function WoWGrok.Send(text, allow)
	local c = ActiveChat()
	...
	if c.pendingId then
		...
		if not c.pendingId then
			...
			if text ~= "" then
				...
				-- Empty Enter/Refresh while pending: check Inbox via reload/slot.
				if UseReloadTransport() then
					SafeReload()
				else
					TryLoadSlot("manual")
	...
	if not WoWGrok.IsConnected() then
		...
		if not run.connectingAt then WoWGrok.Connect() end
		WoWGrok.Toggle(true)
		return
	end
	...
	-- you send (system notes like "/wow-grok cd" before it don't count).
	...
	if UseReloadTransport() then
		-- Capture paused / reload mode / pixel dead: flush SV outbox via ReloadUI.
		SafeReload()
	else
...
-- Grok conversation. db.forget keeps the id until the bridge acks, so a delete made
...
function WoWGrok.SayHello()
	if UseReloadTransport() then return end
	local now = GetTime()
...
function WoWGrok.SendFromInput()
...
-- The Allow button (upstream Claude Code parity). Grok API chat has no local tool
-- allowlist, so the bridge treats these rules as a no-op; we still clear the denied
-- markers and send a "continue" nudge so the UI stays useful if a reply ever
-- carries a denied list from an older bridge.
function WoWGrok.Allow(chatId, rules)
	local c = FindChat(chatId)
	if not c or c.pendingId or not rules or #rules == 0 then return end
	if db.activeChat ~= c.id then WoWGrok.SwitchChat(c.id) end
	for _, m in ipairs(c.history) do m.denied = nil end
	AddHistory(c, "system", "Allowed: " .. table.concat(rules, ", "))
	WoWGrok.Send("Those actions are allowed now. Continue from where you left off.", rules)
end
...
function WoWGrok.SwitchChat(id)
	local c = FindChat(id)
	if not c then return end
...
function WoWGrok.NewChat(name)
	local c = AddChat(name and name ~= "" and name or nil)
	if not c then
		local a = ActiveChat()
		AddHistory(a, "system", "Chat limit reached (" .. MAX_CHATS .. "). Delete one first with /wow-grok delete.")
		WoWGrok.Render()
		return
	end
	WoWGrok.SwitchChat(c.id)
	WoWGrok.Toggle(true)
end

-- Folder this chat's Grok conversation is associated with (metadata only; API chat has no local cwd tools). Empty (or "-" / "default") = the bridge's
-- default. Relative paths are resolved by the bridge against that default.
function WoWGrok.SetFolder(rest, c)
	c = c or ActiveChat()
	if not c then return end
	rest = Trim(rest or "")
	...
	if rest ~= "" then
		...
		AddHistory(c, "system", "cwd set to " .. rest .. note .. (changed and #c.history > 1 and "; the next message starts a fresh Grok conversation there" or ""))
	elseif c.cwd ~= "" then
		c.cwd = ""
		AddHistory(c, "system", "cwd reset to the bridge's default: " .. base)
	else
		AddHistory(c, "system", "cwd is the bridge's default: " .. base .. " (/wow-grok cd <folder>, or right-click the chat and pick Folder, to change)")
	end
	WoWGrok.Render()
end

StaticPopupDialogs["WOWGROK_FOLDER"] = {
	text = "Folder for this chat\n\nRelative to the bridge's folder (%s), ~, or a full path.\nEmpty = the bridge's default. Changing it starts a fresh Grok conversation.",
	button1 = OKAY,
	button2 = CANCEL,
	hasEditBox = 1,
	editBoxWidth = 320,
	...
	OnAccept = function(dialog, data)
		...
		if chat and box then WoWGrok.SetFolder(box:GetText(), chat) end
	end,
	EditBoxOnEnterPressed = function(box)
		local dialog = box:GetParent()
		StaticPopupDialogs["WOWGROK_FOLDER"].OnAccept(dialog, dialog.data)
		dialog:Hide()
	end,
...
function WoWGrok.FolderPrompt(id)
	local c = (id and FindChat(id)) or ActiveChat()
	if not c then return end
	StaticPopup_Show("WOWGROK_FOLDER", run.bridgeCwd or "unknown until connected", nil, { id = c.id, cwd = c.cwd })
end

StaticPopupDialogs["WOWGROK_RENAME"] = {
	...
	EditBoxOnEnterPressed = function(box)
		local dialog = box:GetParent()
		StaticPopupDialogs["WOWGROK_RENAME"].OnAccept(dialog, dialog.data)
		dialog:Hide()
	end,
	EditBoxOnEscapePressed = function(box)
		box:GetParent():Hide()
...
function WoWGrok.RenamePrompt(id)
	local c = (id and FindChat(id)) or ActiveChat()
	if not c then return end
	StaticPopup_Show("WOWGROK_RENAME", nil, nil, { id = c.id, name = c.name })
end
WoWGrok.RenameActive = WoWGrok.RenamePrompt
...
function WoWGrok.DeleteChat(id)
	local c, idx = nil, nil
	...
	if db.activeChat == c.id then
		WoWGrok.SwitchChat(db.chats[math.min(idx, #db.chats)].id)
	else
		WoWGrok.RenderChatList()
	end
end

-- The trash can on a chat row asks first; /wow-grok delete does not.
StaticPopupDialogs["WOWGROK_DELETE"] = {
	...
	hideOnEscape = true,
	OnAccept = function(dialog, data)
		if data then WoWGrok.DeleteChat(data.id) end
	end,
}

function WoWGrok.ConfirmDelete(id)
	local c = (id and FindChat(id)) or ActiveChat()
	if not c then return end
	StaticPopup_Show("WOWGROK_DELETE", Display(c.name), nil, { id = c.id })
end
...
function WoWGrok.UpdateStatus()
	...
	if c and c.pendingId then
		...
		if mode == "pixel" then
			...
			elseif rec and not rec.acked then
				...
				local state = WoWGrok.BridgeState()
			...
			-- Reload / capture-paused path: first reload flushes outbox; second checks Inbox.
			s = "Waiting for reply #" .. id .. " — press Refresh or Enter to read Inbox"
			if run.capturePaused then
				s = s .. " (capture paused; reload chat)"
			end
			if db.settings.autoRefresh then
				local w = db.settings.interval or 20
				if run.capturePaused or mode ~= "pixel" then w = math.min(w, 8) end
				s = s .. "; auto reload on next keypress after " .. w .. "s"
			end
		end
	...
	ui.cwd:SetText("cwd: " .. cwdText .. "   mode: " .. modeLabel)
	if ui.resend then ui.resend:SetShown(c and c.pendingId ~= nil and mode == "pixel") end
	if ui.refresh then ui.refresh:SetShown(UseReloadTransport()) end
	WoWGrok.UpdateMini()
end
...
local function GetBubble(i)
	...
	b.allow:SetScript("OnClick", function(self)
		WoWGrok.Allow(self.chatId, self.rules)
	end)
	b.allow:Hide()
	-- FontStrings can't be selected, so a click opens the message in the copy box.
	b:EnableMouse(true)
	b:SetScript("OnMouseUp", function(self, button)
		if button == "LeftButton" and self.text and self.text ~= "" then WoWGrok.ShowCopy(self.text) end
	end)
	ui.bubbles[i] = b
	return b
end

function WoWGrok.Render()
	local c = ActiveChat()
...
-- Copy box (/wow-grok copy): a selectable EditBox with the last reply pre-highlighted for Ctrl+C.
function WoWGrok.ShowCopy(text)
	if not ui.copy then
		local cf = CreateFrame("Frame", "WoWGrokCopy", UIParent, "BackdropTemplate")
		cf:SetSize(560, 320)
		cf:SetPoint("CENTER")
		...
		tinsert(UISpecialFrames, "WoWGrokCopy")
		...
		local sc = CreateFrame("ScrollFrame", "WoWGrokCopyScroll", cf, "UIPanelScrollFrameTemplate")
		sc:SetPoint("TOPLEFT", cf, "TOPLEFT", 14, -36)
		sc:SetPoint("BOTTOMRIGHT", cf, "BOTTOMRIGHT", -32, 14)
		local eb = CreateFrame("EditBox", "WoWGrokCopyBox", sc)
...
function WoWGrok.RenderChatList()
...
local ECHO_DEFAULT = 4000 -- characters of a reply to print into the game chat ("/wow-grok echo <n>")

local function ChatLinks(chat)
	return "  |Hgrok:reply:" .. chat.id .. "|h|cff55ff55[reply]|r|h |Hgrok:open:" .. chat.id .. "|h|cff7ec8ff[open]|r|h"
end
...
local function EchoToChat(chat, text)
	...
	local prefix = "|cff7ec8ff[Grok · " .. Display(chat.name) .. "]|r "
...
function WoWGrok.Notify(chat, text)
	...
	if UIErrorsFrame then
		UIErrorsFrame:AddMessage("Grok replied in " .. Display(chat.name), 0.5, 0.8, 1, 1)
	end
end

-- /r goes to Grok when Grok was the last one to message you, exactly like
-- whisper reply, and the box shows a "To Grok [chat]:" header while you type.
--
-- The chat type underneath is left alone (a custom type would leak into chat
-- settings); instead the box remembers a Grok target, the header is repainted
...
local GROK_R, GROK_G, GROK_B = 0.49, 0.78, 1.0

local function PaintGrokHeader(eb, chat)
	local header = _G[eb:GetName() .. "Header"]
	local suffix = _G[eb:GetName() .. "HeaderSuffix"]
	if not header then return end
	eb.grokPainting = true
	eb:UpdateHeader() -- lay out normally first, then repaint
	eb.grokPainting = nil
	header:SetWidth(0)
	header:SetText("To Grok [" .. Display(chat.name) .. "]: ")
	header:SetTextColor(GROK_R, GROK_G, GROK_B)
	if suffix then suffix:Hide() end
	eb:SetTextInsets(15 + header:GetWidth(), 13, 0, 0)
	eb:SetTextColor(GROK_R, GROK_G, GROK_B)
end

local function SendBoxToGrok(eb)
	local chat = FindChat(eb.grokTarget)
	local text = Trim(eb:GetText() or "")
	eb.grokTarget = nil
	eb:ClearChat()
	if chat and db.activeChat ~= chat.id then WoWGrok.SwitchChat(chat.id) end
	if text ~= "" then
		WoWGrok.Send(text)
	else
		WoWGrok.Toggle(true)
...
local function HookReplyCommand()
	for i = 1, (NUM_CHAT_WINDOWS or 10) do
		...
		if eb and eb.ProcessChatType and not eb.grokReplyHooked then
			eb.grokReplyHooked = true
			...
			eb.ProcessChatType = function(self, msg, index, send, ...)
				...
				if not (db and run.lastMessenger == "grok") then
					return origProcess(self, msg, index, send, ...)
				end
				local chat = FindChat(run.lastReplyChat) or ActiveChat()
				if send == 1 then
					self:SetText(msg or "")
					self.grokTarget = chat and chat.id
					SendBoxToGrok(self)
					return true
				end
				self.grokTarget = chat and chat.id
				self:SetText(msg or "")
				if chat then PaintGrokHeader(self, chat) end
				return true
			end

			-- Enter arrives here; nothing below us ever sees a Grok-targeted box.
			for _, name in ipairs({ "SendMessage", "SendText" }) do
				local orig = eb[name]
				if orig then
					eb[name] = function(self, ...)
						if self.grokTarget then
							SendBoxToGrok(self)
							return
						end
						return orig(self, ...)
			...
			-- Anything that repaints the header normally (Tab, /s, sticky reset) ends Grok mode.
			hooksecurefunc(eb, "UpdateHeader", function(self)
				if not self.grokPainting then self.grokTarget = nil end
			end)
...
hooksecurefunc("SetItemRef", function(link)
	local action, chatId = tostring(link):match("^grok:(%a+):(%w+)")
	if not action or not db then return end
	if FindChat(chatId) then WoWGrok.SwitchChat(chatId) end
	WoWGrok.Toggle(true)
...
local function BuildUI()
	...
	local f = CreateFrame("Frame", "WoWGrokFrame", UIParent, "BackdropTemplate")
	ui.frame = f
	...
	f:Hide()
	tinsert(UISpecialFrames, "WoWGrokFrame")
	...
	mini:SetScript("OnClick", function() WoWGrok.Minimize(true) end)
	mini:SetScript("OnEnter", function(self)
		GameTooltip:SetOwner(self, "ANCHOR_LEFT")
		GameTooltip:SetText("Minimize to the small bar  (Esc)")
		GameTooltip:AddLine("Grok keeps working; the bar shows when a reply lands.", 0.8, 0.8, 0.8, true)
		GameTooltip:Show()
	end)
	...
	local newBtn = MakeButton(panel, "+ New chat", PANEL_W - 16, function() WoWGrok.NewChat() end)
	...
	local menu = CreateFrame("Frame", "WoWGrokChatMenu", f, "BackdropTemplate")
	...
	MenuItem("Rename...", 1, WoWGrok.RenamePrompt)
	MenuItem("Folder...", 2, WoWGrok.FolderPrompt)
	...
	function WoWGrok.ShowChatMenu(chatId, anchor)
		local c = FindChat(chatId)
		if not c then return end
	...
	for i = 1, MAX_CHATS do
		...
		b.del:SetAlpha(0.6)
		b.del:SetScript("OnClick", function() WoWGrok.ConfirmDelete(b.chatId) end)
		...
		b:SetScript("OnClick", function(self, button)
			if button == "RightButton" then
				WoWGrok.ShowChatMenu(self.chatId, self)
			else
				WoWGrok.SwitchChat(self.chatId)
			end
		end)
		b:SetScript("OnDoubleClick", function(self)
			WoWGrok.SwitchChat(self.chatId)
			WoWGrok.RenamePrompt(self.chatId)
		end)
		b:Hide()
		ui.chatButtons[i] = b
	end
	...
	local scroll = CreateFrame("ScrollFrame", "WoWGrokScroll", f, "UIPanelScrollFrameTemplate")
	scroll:SetPoint("TOPLEFT", panel, "TOPRIGHT", 8, 0)
	scroll:SetPoint("BOTTOMRIGHT", f, "BOTTOMRIGHT", -32, 110)
	ui.scroll = scroll

	local content = CreateFrame("Frame", "WoWGrokContent", scroll)
	content:SetSize(500, 1)
	scroll:SetScrollChild(content)
	ui.content = content
	ui.bubbles = {}
	scroll:HookScript("OnSizeChanged", function(self, w, h)
		if ui.frame:IsShown() then WoWGrok.Render() end
	end)
	...
	local inScroll = CreateFrame("ScrollFrame", "WoWGrokInputScroll", inputBg, "UIPanelScrollFrameTemplate")
	inScroll:SetPoint("TOPLEFT", inputBg, "TOPLEFT", 8, -6)
	inScroll:SetPoint("BOTTOMRIGHT", inputBg, "BOTTOMRIGHT", -24, 6)

	local input = CreateFrame("EditBox", "WoWGrokInput", inScroll)
	input:SetMultiLine(true)
	input:SetAutoFocus(false)
	input:SetFontObject(ChatFontNormal)
	input:SetMaxLetters(0)
	input:SetSize(500, 40)
	input:SetScript("OnEnterPressed", function() WoWGrok.SendFromInput() end)
	...
	local send = MakeButton(f, "Send", SEND_W, WoWGrok.SendFromInput)
	send:SetHeight(30)
	send:SetPoint("LEFT", inputBg, "RIGHT", 6, 0)
	ui.send = send

	-- Connect stands in for Send until the bridge has been seen (see UpdateConnect).
	local connect = MakeButton(f, "Connect", SEND_W, WoWGrok.Connect)
	...
	local resend = MakeButton(f, "Resend", 70, WoWGrok.Resend)
	resend:SetPoint("LEFT", clear, "RIGHT", 6, 0)
	resend:Hide()
	ui.resend = resend

	-- A named, always-present button so a keybinding can click it (see /wow-grok bind).
	local hotkey = CreateFrame("Button", "WoWGrokRefreshButton", UIParent)
	hotkey:SetSize(1, 1)
	...
	local m = CreateFrame("Frame", "WoWGrokMini", UIParent, "BackdropTemplate")
	ui.mini = m
	m:SetSize(250, 30)
	if s.miniPoint then
	...
	mclose:SetScript("OnClick", function() WoWGrok.Toggle(false) end)
	mclose:SetScript("OnEnter", function(self)
		GameTooltip:SetOwner(self, "ANCHOR_LEFT")
		GameTooltip:SetText("Quit: hide completely (/wow-grok brings it back)")
		GameTooltip:Show()
	end)
	mclose:SetScript("OnLeave", function() GameTooltip:Hide() end)
end

function WoWGrok.Toggle(show)
...
function WoWGrok.Minimize(mini)
	if not ui.frame then return end
...
local HELP = table.concat({
	"/wow-grok                        toggle the window (/grok works too)",
	"/wow-grok mini                   collapse to the small bar (click the bar to expand)",
	"/wow-grok hide                   hide the window completely",
	"/ai <text>                         send <text> to the current chat straight from the game chat box",
	"/r <text>                          replies to Grok when Grok was the last to message you (else normal whisper reply)",
	"/wow-grok <text>                 same as /ai",
	"/wow-grok echo full|short|off|<chars>   how much of each reply to print in the game chat",
	"/wow-grok longchat on|off        let the game chat box take 4000 characters (for long /ai messages)",
	"/wow-grok new [name]             start a new chat (its own Grok conversation, like a new terminal)",
	"/wow-grok chat <n|name>          switch chats (or click one in the left panel)",
	"/wow-grok rename [name]          rename the current chat (no name = dialog; right-clicking the chat in the left panel offers it too)",
	...
	"/wow-grok reload                 reload now (also frees the slot pool)",
	"/wow-grok cancel                 stop waiting on this chat's reply",
	"/wow-grok copy                   open the last reply in a selectable box for Ctrl+C",
	"/wow-grok bind <key>             hotkey: checks for a reply while waiting, else toggles the window",
	"/wow-grok auto on|off            reload-mode only: auto-reload on your next keypress after the interval",
	"/wow-grok signal on|off          the cheap sound-file readiness check (off if it spams errors)",
	"/wow-grok slots                  how many reply slots are still free this session",
	"/wow-grok diag                   transport diagnostics (is the cheap sound-file channel working?)",
	"/wow-grok clear                  clear this chat's transcript",
}, "\n")

-- /ai <text>: send straight from the game chat box (like /r, but for Grok).
SLASH_GROKASK1 = "/ai"
SLASH_GROKASK2 = "/ask"
SlashCmdList["GROKASK"] = function(msg)
	msg = Trim(msg or "")
	if msg == "" then
		WoWGrok.Toggle(true)
	else
...
SLASH_WOWGROK2 = "/grok"
SlashCmdList["WOWGROK"] = function(msg)
	msg = Trim(msg or "")
	...
	elseif cmd == "mini" or cmd == "min" then
		WoWGrok.Minimize(true)
	elseif cmd == "new" then
		WoWGrok.NewChat(rest)
	...
	elseif cmd == "rename" then
		if rest ~= "" then
			...
			WoWGrok.RenameActive()
		end
		WoWGrok.Toggle(true)
	elseif cmd == "delete" then
		WoWGrok.DeleteChat()
	elseif cmd == "cd" then
		WoWGrok.SetFolder(rest, c)
		WoWGrok.Toggle(true)
	...
	elseif (cmd == "context" or cmd == "ctx") and (rest == "" or rest:lower() == "on" or rest:lower() == "off") then
		...
		if rest == "on" or rest == "off" then
			...
			run.contextSent = nil
			run.lastHelloAt = nil
			if WoWGrok.IsConnected() then WoWGrok.SayHello() end
		end
	...
	elseif cmd == "hide" or cmd == "quit" then
		WoWGrok.Toggle(false)
	elseif cmd == "copy" then
		for i = #c.history, 1, -1 do
			if c.history[i].role == "grok" then
				WoWGrok.ShowCopy(c.history[i].text)
				break
			end
		end
	...
	elseif cmd == "bind" then
		...
		if key ~= "" and not InCombatLockdown() then
			SetBinding(key, "CLICK WoWGrokRefreshButton:LeftButton")
...
ev:SetScript("OnEvent", function(self, event, arg1)
	...
	elseif event == "PLAYER_LOGIN" then
		...
		C_Timer.After(3, WoWGrok.SayHello)
	elseif event == "PLAYER_REGEN_ENABLED" then
		if WoWGrok.reloadAfterCombat then
			WoWGrok.reloadAfterCombat = nil
			ReloadUI()
		elseif db then
			WoWGrok.ArmAutoRefresh()
		end
