...
| Capture errors on Mac / screen-record prompt loops | Enable **WoWGrok** under Screen Recording, then **Quit and reopen** WoWGrok (toggle alone is not enough). v0.1.15+ stops the Settings spam loop when screencapture fails after an unsigned upgrade (capture exits; Connect/presence still work). v0.1.16+ also persists `capture.permissionPaused` so capture never re-spawns (no system Screen Recording sheet on Forever launch). Clear that flag in Application Support config.json after fixing TCC. First launch may prompt Screen Recording and must create a WoWGrok row in System Settings; the in-app sheet shows unless a real capture probe says granted (v0.1.12+; window-list alone is not enough). Use windowed/borderless. |
