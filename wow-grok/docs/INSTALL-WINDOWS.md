...
3. Double-click; enter your xAI API key; confirm your WoW **Interface/AddOns** folder. After Forever is running with **WoW Grok** enabled, type `/wow-grok` or `/grok` in game chat.
4. Leave **WoWGrok.exe** running while you play. A **system tray** (notification area) icon shows **Running**. When you are done and want to stop the bridge, open that tray icon and choose **Quit** — Quit closes the app; it is not part of opening or using the AddOn.
