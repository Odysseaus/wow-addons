...
`capture.ps1` (Windows) finds the game window by process name, captures the client area's top-left 800×192 px with GDI (`CopyFromScreen`, DPI-aware), samples the center pixel of each cell, and validates magic, length and checksum. On macOS, experimental `capture-mac.js` uses AppleScript to locate the window and `screencapture -R` for the same region; Retina displays report bounds in points and a PNG in pixels, so cells are sampled at `cellPx * backingScaleFactor`. Both print one JSON line per new message and rate-limited warnings when a frame is seen but rejected. `bridge.js` restarts the capture process if it exits.
