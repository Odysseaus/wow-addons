...
```
   ...
   ┌──────────────────────────┐                    ┌─────────────────────────────┐
   │ WoWGrok addon         │  pixels on screen  │ capture.ps1 (Windows) or    │
   │  draws message strip ────┼───────────────────▶│  capture-mac.js (macOS)     │
   ...
   │                          │                    │    response_id + history)   │
   │                          │                    │        │                    │
   │  LoadAddOn(WoWGrok_S…)│  files on disk     │        ▼                    │
...
- `flags` — `n` = start a fresh Grok conversation; `h` = hello (announce the session token, no prompt); `d` = the chat was deleted in game: drop its transcript and Grok response id, no prompt (the addon keeps the id in `db.forget` and resends it with each hello until the bridge acks); `allow=Rule1,Rule2` = ignored (Grok API chat has no local tool allowlist).
...
`capture.ps1` (Windows) finds the game window by process name, captures the client area's top-left 800×192 px with GDI (`CopyFromScreen`, DPI-aware), samples the center pixel of each cell, and validates magic, length and checksum. On macOS, `bridge_py/capture_mac.py` locates the window via JXA (`kCGWindowNumber` + bounds) and prefers an in-process `CGWindowListCreateImage` strip for that window id; `screencapture -R` is fallback only. Retina: bounds in points, PNG in pixels; cells sampled at `cellPx * backingScaleFactor` (preferring the screen that owns the window). Both print one JSON line per new message and rate-limited warnings when a frame is seen but rejected. The Python bridge restarts capture on non-42 exits; exit 42 + `permissionPaused` pauses until a CG window resume smoke succeeds.

Exclusive fullscreen blocks capture; borderless/windowed works. HDR was not tested. macOS needs Screen Recording permission.

## Inbound: load-on-demand slots

`install-slots.js` creates `WoWGrok_S001` … `WoWGrok_S200`, each a `## LoadOnDemand: 1` addon with a single `Inbox.lua`. `C_AddOns.LoadAddOn` reads that file from disk at load time; each slot can be loaded once per UI session, and `/reload` unloads them all.
...
WoWGrok_SlotData = {
  ...
  replies = { { chat = "...", id = 12, status = "working"|"done"|"error", text = "...", cwd = "...", session = "<xAI response id>" }, … },
...
The same content is written to `WoWGrok/Inbox.lua`, which the game reads on `/reload` — the fallback path and the only path in `mode reload`.
...
| `act/NNN/kk.wav` | Grok's k-th progress beat on message NNN → live "14 actions, last 6 s ago" without spending a slot |
...
- **Folders:** the default folder is `--project`, else the folder the bridge was started from (the `wow-grok` command, see README), else `defaultCwd` in the config; it is reported to the addon as `cwd` in every slot file. A chat's folder is resolved against it (`realms` → `<default>/realms`; empty = the default). `state.json` remembers the folder each conversation ran in and a chat that changed folder starts a new Grok conversation.
- **Jobs:** one xAI Grok API request per chat, up to `maxParallel` at once, queued per chat beyond that. `bridge/xai.js` posts to `/v1/responses` with `previous_response_id` when `state.sessions` has a prior response id; if that id fails, it retries once with local `state.history`. Heartbeat files flip while the request is in flight; the assistant `output_text` becomes the reply. Response ids are stored per chat id in `state.json`.
- **Permissions:** stub. Grok API chat has no local tools. `allowRules` logs a note and does nothing; `denied` is never set on a finish record. The in-game Allow button is a no-op.
...
`WoWGrok.lua` is a single file; sections in order: helpers, reload fallback, pixel strip, signals and slots, sending, chats, rendering, UI, slash commands, events. `tests/addon_test.js` runs it in a Lua VM with a stub client (`tests/wow_stub.lua`) through a whole session: login, hello, a message decoded off the strip, a slot reply, Allow, restore.
...
- **Game chat:** replies are printed line by line under `[Grok · name]` with `[reply]`/`[open]` hyperlinks (`|Hgrok:…|h`, handled via `hooksecurefunc("SetItemRef")`). `/ai` sends from the chat box. `/r` is handled by wrapping `ProcessChatType`, `SendMessage` and `SendText` on each chat edit box: when Grok was the last messenger, the box shows a repainted header and Enter routes to Grok with the box cleared first, so the game never sends anything; the underlying chat type is untouched, and `UpdateHeader`/`ClearChat` hooks end Grok mode on Tab, `/s`, Esc or a real incoming whisper.
...
- 200 slots per UI session. Each reply costs one slot when the readiness signal works, about four otherwise; `/wow-grok reload` resets the pool.
...
- Windows (PowerShell capture) and experimental macOS (`capture-mac.js`, Screen Recording, Retina). NTFS is not required on Mac.
