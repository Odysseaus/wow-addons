...
// `node bridge.js --inject "..."` calls the xAI Grok API and must publish the
// reply into every slot, Inbox.lua, and flip the signal / heartbeat files.
// Needs XAI_API_KEY (or config apiKey).
'use strict';
...
fs.mkdirSync(path.join(S, 'addons', 'WoWGrok'), { recursive: true });
fs.mkdirSync(path.join(S, 'proj'), { recursive: true });
for (const f of ['bridge.js', 'protocol.js', 'xai.js', 'install-slots.js', 'capture.ps1', 'capture-mac.js']) fs.copyFileSync(path.join(SRC, f), path.join(S, f));
fs.writeFileSync(path.join(S, 'addons', 'WoWGrok', 'WoWGrok.toc'), '## Interface: 16001\n');

const cfg = JSON.parse(fs.readFileSync(path.join(SRC, 'config.example.json'), 'utf8'));
cfg.addonDir = path.join(S, 'addons');
cfg.inboxFile = path.join(S, 'addons', 'WoWGrok', 'Inbox.lua');
...
const r = spawnSync(process.execPath, ['bridge.js', '--inject', 'Reply with exactly the word PONG and nothing else.'], { cwd: S, encoding: 'utf8', env, timeout: 180000 });
...
let ok = true;
for (let i = 1; i <= 5; i++) {
  const d = readLua(path.join(S, 'addons', 'WoWGrok_S00' + i, 'Inbox.lua'), 'WoWGrok_SlotData');
...
const inbox = readLua(cfg.inboxFile, 'WoWGrok_Inbox');
const ir = (inbox.replies || [])[0] || {};
console.log(`Inbox.lua: id=${ir.id} status=${ir.status} text=${JSON.stringify(ir.text)}`);
ok = ok && ir.text === 'PONG';
const size = f => fs.statSync(path.join(S, 'addons', 'WoWGrok', f)).size;
