// Round-trip test: the addon's real Codec.lua (run in a Lua VM) -> PNG -> capture decoder.
// Windows: capture.ps1. Elsewhere: capture-mac.js --test-image (pure PNG inflate).
'use strict';
...
const CODEC = path.join(__dirname, '..', 'addon', 'WoWGrok', 'Codec.lua');
const CAPTURE_PS1 = path.join(__dirname, '..', 'bridge', 'capture.ps1');
const CAPTURE_MAC = path.join(__dirname, '..', 'bridge', 'capture-mac.js');
...
function encodeWithLua(id, payload) {
  ...
  const code = fs.readFileSync(CODEC, 'utf8') +
    `\nlocal cells, n = WoWGrok_Codec.Encode(${id}, ${lit})\n` +
...
function decode(file) {
  const out = process.platform === 'win32'
    ? execFileSync('powershell.exe', ['-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', CAPTURE_PS1, '-TestImage', file], { encoding: 'utf8' })
    : execFileSync(process.execPath, [CAPTURE_MAC, '--cell', String(CELL), '--cells', String(CELLS), '--max-rows', String(MAXROWS), '--test-image', file], { encoding: 'utf8' });
