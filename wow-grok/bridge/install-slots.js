...
if (!fs.existsSync(path.join(addons, 'WoWGrok', 'WoWGrok.toc'))) {
  console.error('WoWGrok addon not found under ' + addons);
  process.exit(1);
}
...
for (let i = 1; i <= N; i++) {
  const name = 'WoWGrok_S' + String(i).padStart(3, '0');
  const dir = path.join(addons, name);
  ensure(path.join(dir, name + '.toc'), [
    '## Interface: ' + iface,
    '## Title: WoW Grok slot ' + String(i).padStart(3, '0'),
  ...
  ensure(path.join(addons, 'WoWGrok', 'sig', String(i).padStart(3, '0') + '.wav'), '');
  ensure(path.join(addons, 'WoWGrok', 'ack', String(i).padStart(3, '0') + '.wav'), '');
  // Heartbeat files: one per Grok progress beat, flipped valid by the bridge as it works.
  for (let k = 1; k <= ACT; k++) {
    ensure(path.join(addons, 'WoWGrok', 'act', String(i).padStart(3, '0'), String(k).padStart(2, '0') + '.wav'), '');
  }
}

// Presence files: the bridge flips one every 30 s so the game can show "connected".
for (let k = 1; k <= PRESENCE; k++) {
  ensure(path.join(addons, 'WoWGrok', 'presence', String(k).padStart(4, '0') + '.wav'), '');
}

// Control files for the addon's self-test: one always empty, one always valid.
ensure(path.join(addons, 'WoWGrok', 'ctl', 'empty.wav'), '');
ensure(path.join(addons, 'WoWGrok', 'ctl', 'valid.wav'), require('./protocol').SILENT_WAV);
