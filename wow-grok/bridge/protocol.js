...
// Grok conversations (xAI response ids) are keyed by chat id, which survives an addon data reset.
...
// Flags field: ';'-separated tokens. "n" = fresh Grok conversation, "h" = hello
// (no prompt), "d" = the player deleted this chat: forget its transcript and
// response id (no prompt), "allow=Rule1,Rule2" = ignored by the Grok API bridge.
...
function describeToolUse(block) {
  ...
  const base = p => path.basename(String(p || '').replace(/\\/g, '/'));
  switch (block.name) {
...
function luaTable(globalName, records, opts = {}) {
  ...
  const lines = [
    '-- Written by the wow-grok bridge (bridge/bridge.js). Do not edit by hand.',
    `${globalName} = {`,
